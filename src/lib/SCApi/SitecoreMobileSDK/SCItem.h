...
/**
 ...
 @return SCAsyncOp block. Call it to get the expected result. The SCAsyncOpResult handler's result is NSDictionary of field's values by field's names or SCError if error happens.
 */
...
/**
 ...
 To read all fields choose nil or the empty set if you don't need to read any field
 @return SCAsyncOp block. Call it to get the expected result. The SCAsyncOpResult handler's result is NSDictionary of SCField objects by field's names or SCError if error happens.
 */
- (SCAsyncOp)fieldsReaderForFieldsNames:(NSSet *)fieldNames;

/**
 Used to load all item's children, see -[SCItem allChildren]
 @return SCAsyncOp block. Call it to get the expected result. The SCAsyncOpResult handler's result is NSArray of SCItem objects or SCError if error happens.
 */
- (SCAsyncOp)childrenReader;

/**
 Used to save all item's changed fields
 @return SCAsyncOp block. Call it to save item. The SCAsyncOpResult handler's result is SCItem object or SCError if error happens.
 */
- (SCAsyncOp)saveItem;

/**
 Used to remove given item
 @return SCAsyncOp block. Call it to remove item. The SCAsyncOpResult handler's result is NSNumber object( always equal to "1" ) or SCError if error happens.
 */

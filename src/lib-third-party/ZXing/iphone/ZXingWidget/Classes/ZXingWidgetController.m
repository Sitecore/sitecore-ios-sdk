...
- (CGImageRef)CGImageRotated90:(CGImageRef)imgRef CF_RETURNS_RETAINED
{
...
- (CGImageRef)CGImageRotated180:(CGImageRef)imgRef CF_RETURNS_RETAINED
{
...
{ 
  ...
  CGImageRef origCapture = CGBitmapContextCreateImage(newContext); 
  ...
  if (oneDMode) {
    ...
    // do a rotate
    CGImageRef croppedImg = CGImageCreateWithImageInRect(origCapture, cropRect);
    CGImageRelease( origCapture );
    CGImageRef capture1 = [self CGImageRotated90:croppedImg];
    CGImageRef capture = [self CGImageRotated180:capture1];
    CGImageRelease( capture1 );
  ...
  {
    float height = CGImageGetHeight(origCapture);
    float width = CGImageGetWidth(origCapture);
  ...
  CGImageRef newImage = CGImageCreateWithImageInRect(origCapture, cropRect);
  CGImageRelease(origCapture);
